...
export function buildFilename(prompt: string, format: ExportFormat): string {
  const cleaned = (prompt || "dmoop-output")
    .replace(/[^\w\s-]/g, "")
    .trim()
    .split(/\s+/)
    .slice(0, 6)
    .join("-")
    .toLowerCase() || "dmoop-output";
...
async function exportXlsx(content: string, filename: string): Promise<void> {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "DMOOP");
...
async function exportPptx(content: string, filename: string): Promise<void> {
  ...
  if (current) slides.push(current);
  if (slides.length === 0) slides.push({ title: "DMOOP Output", bullets: [content.slice(0, 800)] });
  ...
  for (const s of slides) {
    ...
    slide.addText("Generated by DMOOP", {
...
<html lang="en"><head><meta charset="utf-8"><title>DMOOP Output</title>
